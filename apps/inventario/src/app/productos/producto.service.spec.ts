...
describe('ProductoService (unit)', () => {
  ...
  test('lanza BadRequestException cuando faltan datos de medicamento', async () => {
    ...
    await expect(service.createProducto(dto, jwt)).rejects.toBeInstanceOf(
      BadRequestException
    );
  ...
  describe('ProductoService - obtenerProductosDeUnaRegion (unit)', () => {
    ...
    test('devuelve lista mapeada correctamente', async () => {
      ...
      for (let i = 0; i < repoProducts.length; i++) {
        expect(result[i].productoRegionalId).toBe(
          repoProducts[i].detalleRegional.id
        );
        ...
        expect(result[i].descripcion).toBe(
          repoProducts[i].productoGlobal.descripcion
        );
        expect(result[i].tipo).toBe(
          repoProducts[i].productoGlobal.tipoProducto
        );
  ...
  describe('findById (unit)', () => {
    ...
    test('Arrange/Act/Assert: lanza NotFoundException cuando repositorio devuelve null', async () => {
      ...
      // Act & Assert
      await expect(
        service.findById('non-existent-id', jwt)
      ).rejects.toBeInstanceOf(NotFoundException);
    ...
    test('Arrange/Act/Assert: lanza NotFoundException cuando el producto pertenece a otra región', async () => {
      ...
      // Act & Assert
      await expect(service.findById('prod-uuid-2', jwt)).rejects.toBeInstanceOf(
        NotFoundException
      );
  ...
  describe('obtenerProductosEnBodega (unit)', () => {
    ...
    test('Arrange/Act/Assert: devuelve lista cuando el repositorio tiene productos', async () => {
      // Arrange
      const bodegaId = 'bodega-123';
      const repoProducts: any[] = [
        { sku: 'SKU-1', cantidad: 10, bodegaId },
        { sku: 'SKU-2', cantidad: 5, bodegaId },
      ];
      mockRepo.findByBodega.mockResolvedValue(repoProducts);

      // Act
      const result = await service.obtenerProductosEnBodega(bodegaId);

      // Assert
      expect(mockRepo.findByBodega).toHaveBeenCalledTimes(1);
      expect(mockRepo.findByBodega).toHaveBeenCalledWith(bodegaId);
      expect(Array.isArray(result)).toBe(true);
      expect(result).toHaveLength(repoProducts.length);
      expect(result).toEqual(repoProducts);
    });

    test('Arrange/Act/Assert: devuelve array vacío cuando el repositorio devuelve []', async () => {
      // Arrange
      const bodegaId = 'empty-bodega';
      mockRepo.findByBodega.mockResolvedValue([]);
    ...
    test('Arrange/Act/Assert: propaga errores del repositorio', async () => {
      // Arrange
      const bodegaId = 'err-bodega';
      mockRepo.findByBodega.mockRejectedValue(new Error('DB error'));

      // Act & Assert
      await expect(service.obtenerProductosEnBodega(bodegaId)).rejects.toThrow(
        'DB error'
      );
      expect(mockRepo.findByBodega).toHaveBeenCalledWith(bodegaId);
    });
